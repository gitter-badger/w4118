			   ...
			   const struct sched_attr *attr)
{
	...
	if (rt_prio(p->prio))
		...
		p->sched_class = &wrr_sched_class;
		p->wrr.weight = get_task_weight_wrr(p);
	} else
		p->sched_class = &fair_sched_class;
...
SYSCALL_DEFINE1(get_wrr_info, struct wrr_info __user *, user_wrr_info)
{
	int cpu = 0;
	int num_cpus = 0;
	...
	for_each_possible_cpu(cpu) {
		...
		k_wrr_info.nr_running[num_cpus] = rq->wrr.nr_running;
		k_wrr_info.total_weight[num_cpus] = rq->wrr.total_weight;

		num_cpus++;
		raw_spin_unlock(&rq->lock);
	}

	k_wrr_info.num_cpus = num_cpus;

	if (copy_to_user(user_wrr_info, &k_wrr_info, sizeof(struct wrr_info)))
		return -EFAULT;
...
SYSCALL_DEFINE1(set_wrr_weight, int, boosted_weight)
{
	...
	set_boosted_weight(boosted_weight);
...
void sched_move_task(struct task_struct *tsk)
{
	...
	if (tsk->policy == SCHED_WRR)
		tsk->wrr.weight = get_task_weight_wrr(tsk);
	else
